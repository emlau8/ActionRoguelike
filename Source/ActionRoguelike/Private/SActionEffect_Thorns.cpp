...
void USActionEffect_Thorns::StartAction_Implementation(AActor* Instigator)
{
	...
	if (Attributes)
	{
		// Bind to the health changed event
...
void USActionEffect_Thorns::OnHealthChanged(AActor* InstigatorActor, USAttributeComponent* OwningComp, float NewHealth, float Delta)
{
	...
	if (Delta < 0.0f && OwningActor != InstigatorActor)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("Thorns: Original Damage: %f, Reflected Amount: %d"), Delta, ReflectedAmount);
		...
		// Flip to positive, so we don't end up healing ourselfves when passed into damage
